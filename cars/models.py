...
class Car(models.Model):
    ...
    )  # ano de fabricação, exemplo: 2022
    model_year = models.IntegerField(
        blank=True, null=True
    )  # ano do modelo, exemplo: 2023
    plate = models.CharField(
        max_length=10, blank=True, null=True
    )  # placa do carro, exemplo: "ABC-1234"
    value = models.FloatField(
        blank=True, null=True
    )  # numero flutuante exemplo: 1234.56
    photo = models.ImageField(
        upload_to="cars/", blank=True, null=True
    )  # imagem do carro
    bio = models.TextField(blank=True, null=True)  # descrição do carro
